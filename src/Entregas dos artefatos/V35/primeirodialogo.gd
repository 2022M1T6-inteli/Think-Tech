...
func _on_instrucao1_body_entered(body):
	...
	var dialog = Dialogic.start("Instrucao1") # A variável recebe a timeline Instrução1
...
func _on_Area2D2_body_entered(body):
	...
	if body.name == "Player": # Identifica que o player entrou
		...
		var dialog = Dialogic.start("dialogocombate") # A variável recebe a timeline do dialogocombate
...
func _on_Area2D_body_entered(body):
	...
	if body.name == "Player": # Identifica que o player entrou
		...
		var dialog = Dialogic.start("Primeirodialogo") # A variável recebe a timeline Primeirodialogo
